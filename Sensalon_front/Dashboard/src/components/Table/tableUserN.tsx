...
export const TableUserN = ({ encabezados, data, fetch, outofstock, handleEdit }: { encabezados: string[], data: Distributor[] | any, fetch: { (): void } | null, outofstock: string | '', setShowModal: any, showModal: any, handleEdit: (dist: Distributor) => void }) => {
                            ...
                            data?.map((userN: any, index: number) => (
                                <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userN.vcfirstname ? userN.vcfirstname : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userN.vclastname ? userN.vclastname : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userN.vcusername ? userN.vcusername : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userN.vcemail ? userN.vcemail : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userN.dtcreation ? new Date(userN.dtcreation).toLocaleDateString() : 'N/A'}
                                    </td>
