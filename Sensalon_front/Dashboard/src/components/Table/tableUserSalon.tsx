...
export const TableUserSalon = ({ encabezados, data, fetch, outofstock, handleEdit }: { encabezados: string[], data: Distributor[] | any, fetch: { (): void } | null, outofstock: string | '', setShowModal: any, showModal: any, handleEdit: (dist: Distributor) => void }) => {
                            ...
                            data?.map((userSalon: SalonData | any, index: number) => (
                                <tr key={index} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.vcsalonname ? userSalon.vcsalonname : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.vccellphone ? userSalon.vccellphone : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.vcemail ? userSalon.vcemail : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.vcaddress ? userSalon.vcaddress : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.dtopeningtime ? userSalon.dtopeningtime : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.dtdeparturtime ? userSalon.dtdeparturtime : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.service_names_array ? JSON.parse(userSalon.service_names_array).join(', ') : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.vclogo ? <img src={userSalon.vclogo} alt="Logo" className="w-10 h-10" /> : 'N/A'}
                                    </td>
                                    <td className="px-5 py-2 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                        {userSalon.dtCreated ? new Date(userSalon.dtCreated).toLocaleDateString() : 'N/A'}
                                    </td>
