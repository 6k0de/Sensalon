...
export const ModalUsers = ({
  ...
  useEffect(() => {
    if (initialData) {
      if (initialData) {
        const transformed = transformToFormData(initialData);
        setFormData(transformed);
        setChangeRol(transformed.role);
      }
    }
  }, [initialData]);
  ...
  const passwordsMatch = passwordsMatchProvider
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (formData.role === "Salón") {
      formDataSend.append(
        ...
        formData.salonData?.nombreSalon || "",
      );
      formDataSend.append(
        "salonData[telefono]",
        formData.salonData?.telefono || "",
      );
      formDataSend.append(
        "salonData[correo]",
        formData.salonData?.correo || "",
      );
      formDataSend.append(
        "salonData[direccion]",
        formData.salonData?.direccion || "",
      );
      formDataSend.append(
        "salonData[horaApertura]",
        formData.salonData?.horaApertura || "",
      );
      formDataSend.append(
        "salonData[horaCierre]",
        formData.salonData?.horaCierre || "",
      );
      formDataSend.append(
        "salonData[serviciosOfrecidos]",
        formData.salonData?.serviciosOfrecidos || "",
      );
    ...
    if (formData.role === "Distribuidor") {
      formDataSend.append(
        "distributorData[telefono]",
        formData.distributorData?.telefono || "",
      );
      formDataSend.append(
        "distributorData[correo]",
        formData.distributorData?.correo || "",
      );
      formDataSend.append(
        "distributorData[pais]",
        formData.distributorData?.pais || "",
      );
      formDataSend.append(
        "distributorData[estado]",
        formData.distributorData?.estado || "",
      );
      formDataSend.append(
        "distributorData[ciudad]",
        formData.distributorData?.ciudad || "",
      );
      formDataSend.append(
        "distributorData[codigoPostal]",
        formData.distributorData?.codigoPostal || "",
      );
      formDataSend.append(
        "distributorData[direccion]",
        formData.distributorData?.direccion || "",
      );
      formDataSend.append(
        "distributorData[rfc]",
        formData.distributorData?.rfc || "",
      );
      formDataSend.append(
        "distributorData[empresasRelacionadas]",
        formData.distributorData?.empresasRelacionadas || "",
      );
      if (formData.distributorData?.constanciaFiscal) {
        formDataSend.append(
          "constanciaFiscal",
          formData.distributorData.constanciaFiscal,
        );
      }
  ...
  return (
    <>
      <div
        id="crud-modal"
        aria-hidden="true"
        className={`fixed inset-0 z-50 flex items-center justify-center transition-opacity ${show ? "opacity-100" : "opacity-0 pointer-events-none"} duration-300 ease-in-out`}
      >
        <div className="fixed inset-0 bg-[#1d1d1b] bg-opacity-50 transition-opacity duration-300 ease-in-out"></div>
        <div
          className={`relative w-full max-w-6xl max-h-full bg-white rounded-xl shadow dark:bg-gray-700 transform transition-transform ${show ? "scale-100" : "scale-95"} duration-300 ease-in-out`}
        >
          <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
            <h3 className="text-2xl font-bold text-[#1d1d1b] dark:text-white">
              {mode === 1 ? `Editar ${changeRol}` : `Crear Nuevo ${changeRol}`}
            </h3>
            <button
              type="button"
              onClick={onClose}
              className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-xl text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
            >
              <svg
                className="w-3 h-3"
                aria-hidden="true"
          ...
          <form className="p-4 md:p-5" onSubmit={handleSubmit}>
            <div className="grid gap-4 mb-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-9">
              <div className="col-span-2 lg:col-span-3">
                ...
                <select
                  ...
                  value={changeRol}
                  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                >
            ...
            <div className="grid gap-4 mb-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-12">
              ...
              <div className="col-span-2 lg:col-span-3">
                ...
                <input
                  ...
                  onChange={handleInputChange}
                  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-xl focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                  placeholder="farriaga@gmail.com"
                />
              </div>
            </div>
            <hr className="h-px my-8 bg-gray-200 border-0 dark:bg-gray-700" />
            {changeRol === "Distribuidor" ? (
              <DistributorsForms
                formData={formData}
                setFormData={setFormData}
              />
            ) : changeRol === "Salón" ? (
              <SalonForms formData={formData} setFormData={setFormData} />
            ) : (
              <></>
            )}
            ...
            <div className="flex justify-end mt-4">
              <button
                type="submit"
                className="flex text-white items-center mt-5 bg-[#32322f] hover:bg-[#1d1d1b] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-xl text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
              >
                <svg
                ...
                {mode === 1 ? `Guardar Cambios` : `Crear ${changeRol}`}
              </button>
            </div>
          </form>
        </div>
